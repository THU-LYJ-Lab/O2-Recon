...
# dir_snu_code = "/data/huyb/nips-2023/surface_normal_uncertainty"
...
# scannet     
def prepare_neuris_data_from_scannet(dir_scan, dir_neus, dir_snu_code, sample_interval=6, 
                                    b_sample = True, 
                                    b_crop_images = True,
                                    b_generate_neus_data = True,
                                    b_pred_normal = True, normal_method = 'snu',
                                    b_detect_planes = False,
                                    b_with_obj_mask = False,
    ...
    if b_with_inpaint:
        image_folder_name = 'image_filled_masked'
    else:
        image_folder_name = 'image'
    
    # import pdb; pdb.set_trace() 
    H, W, _ = read_image(f'{dir_scan}/{image_folder_name}/0.jpg').shape
    ...
    if W == 1296: 
        ...
        reso_level = 0.975   
    elif W == 1200 and 'replica' in dir_scan:
        path_intrin_color_crop_resize = f'{dir_scan}/../intrinsic_color_crop1200_resize1200.txt'
        origin_size = (1200, 680)
        cropped_size = (1200, 680)
        reso_level = 1.0
    else:
    ...
    if b_sample:
        start_id = 0
        num_images = len(glob.glob(f"{dir_scan}/{image_folder_name}/**.jpg"))
        end_id = num_images
        ScannetData.select_data_by_range(dir_scan, dir_neus, start_id, end_id, sample_interval, 
                                            b_crop_images, cropped_size, b_with_inpaint=b_with_inpaint)
    
    ## copy the semantic.txt file.
    # import pdb; pdb.set_trace()
    shutil.copy(f'{dir_scan}/semantic.txt', f'{dir_neus}/semantic.txt')
    
    ## copy the clip_scores.json file.
    if os.path.exists(f'{dir_scan}/clip_scores.json'):
        shutil.copy(f'{dir_scan}/clip_scores.json', f'{dir_neus}/clip_scores.json')
    
    # prepare neus data
    if b_generate_neus_data:
        dir_neus = dir_neus
        
        if 'replica' in dir_scan:
        ...
        scale = dataset.generate_neus_data(with_obj_mask=b_with_obj_mask)
        scale_depth_dir = f'{dir_neus}/scale_depth'
        depth_dir = f'{dir_neus}/depth'
        if not os.path.exists(scale_depth_dir):
            os.makedirs(scale_depth_dir)
        depth_list = sorted(glob.glob(f'{depth_dir}/*.png'))
        for d in depth_list:
            depth_im = cv2.imread(d, -1).astype(np.float32)
            depth_im /= scale
            depth_im = depth_im.astype(np.uint16)
            cv2.imwrite(os.path.join(scale_depth_dir, d[-8:]), depth_im)

        if b_pred_normal:
            predict_normal(dir_neus, dir_snu_code, normal_method, is_object=is_object, b_with_inpaint=b_pred_normal_full, dir_scan=dir_scan)
...
def prepare_neuris_data_from_scannet_whole(dir_scan, dir_neus, sample_interval=6, 
                                    b_sample = True, 
                                    b_crop_images = True,
                                    b_generate_neus_data = True,
                                    b_pred_normal = True, normal_method = 'snu',
    ...
    if b_sample:
        ...
        num_images = len(glob.glob(f"{dir_scan}/{image_folder_name}/**.jpg"))
        end_id = num_images
        ScannetData.select_data_by_range_for_whole(dir_scan, dir_neus, start_id, end_id, sample_interval, 
    ...
    if b_generate_neus_data:
        ...
        if b_pred_normal:
            predict_normal(dir_neus, normal_method, b_with_inpaint=b_with_inpaint, dir_scan=dir_scan)
...
def predict_normal(dir_neus, dir_snu_code, normal_method = 'snu', is_object = False, b_with_inpaint = False, dir_scan=None):
    ...
    if normal_method == 'snu':
        ...
        if is_object:
            if b_with_inpaint:
                os.system(f'python test.py --pretrained scannet_neuris_retrain --architecture BN --imgs_dir {dir_scan}/image_filled/')
                normal_output_path = f'{dir_scan}/image_filled/results'
            else:
                os.system(f'python test.py --pretrained scannet_neuris_retrain --architecture BN --imgs_dir {dir_scan}/origin_image/')
                normal_output_path = f'{dir_scan}/origin_image/results'
        else:
            os.system(f'python test.py --pretrained scannet_neuris_retrain --architecture BN --imgs_dir {dir_neus}/image/')
            normal_output_path = f'{dir_neus}/image/results'

        if is_object:
            normal_path = f'{dir_scan}/snu_normal'
            if b_with_inpaint:
                mask_path = f'{dir_scan}/mask_filled'
            else:
                mask_path = f'{dir_scan}/mask'
        else:
            normal_path = f'{dir_neus}/snu_normal'
            mask_path = f'{dir_neus}/mask'
        
        # import pdb; pdb.set_trace()
        if not os.path.exists(normal_path):
            os.makedirs(normal_path)
            for name in os.listdir(normal_output_path):
                if name.endswith('_pred_norm.png'):
                    shutil.move(os.path.join(normal_output_path, name), os.path.join(normal_path, name.split('_')[0] + '.png'))
            
            shutil.rmtree(normal_output_path)
        
        save_path = f'{dir_neus}/pred_normal'
        ...
        file_list = os.listdir(normal_path)

        for f in file_list:           
            norm_data = (cv2.imread(os.path.join(normal_path, f)) / 255 - 0.5) * 2

            
            img_idx = int(f[:-4])
            
            mask = cv2.imread(os.path.join(mask_path, f'{img_idx}.png'), cv2.IMREAD_GRAYSCALE)
            norm_data[cv2.resize(mask, (norm_data.shape[1], norm_data.shape[0])) < 127] = -1

            # mask = np.zeros((H, W))
            # mask[inst_mask] = 1.0

            # cv2.imwrite(f"{mask_path}/{f[:4]}.png", (mask * 255).astype(np.uint8))
            np.savez(f"{save_path}/{img_idx:04d}.npz", arr_0=norm_data[...,::-1])
            cv2.imwrite(f"{save_path}/{img_idx:04d}.png", ((norm_data + 1) * 0.5 * 255).astype(np.uint8))
