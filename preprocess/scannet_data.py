...
class ScannetData:
    ...
    @staticmethod
    def select_data_by_range(dir_scan, dir_scan_select, start_id, end_id, interval, b_crop_images, cropped_size = (1248, 936), b_with_inpaint = False):  
        '''
        Args:
        ...
        if b_with_inpaint:
            mask_folder_name = 'mask_filled'
            image_folder_name = 'image_filled_masked'
        else:
            mask_folder_name = 'mask'
            image_folder_name = 'image'
        
        for i in ['image', 'depth', 'pose', 'mask']:
            IOUtils.ensure_dir_existence(f"{dir_scan_select}/{i}/")
        
        if b_with_inpaint:
            IOUtils.ensure_dir_existence(f"{dir_scan_select}/image_filled/")
        
        crop_height_half, crop_width_half = 0, 0
        for idx in range(start_id, end_id, interval):
            # rgb
            path_src = f"{dir_scan}/{image_folder_name}/{idx}.jpg"
            img = cv2.imread(path_src, cv2.IMREAD_UNCHANGED)
            path_src_mask = f"{dir_scan}/{mask_folder_name}/{idx}.png"
            inst_mask = cv2.imread(path_src_mask, cv2.IMREAD_UNCHANGED)
            ...
            if b_crop_images:
                ...
                inst_mask_crop = inst_mask[crop_height_half:height-crop_height_half, crop_width_half:width-crop_width_half]
                assert img_crop.shape[0] == cropped_size[1]
                if 'replica' in dir_scan:
                    img = cv2.resize(img_crop, (1200, 680), interpolation=cv2.INTER_LINEAR)
                    inst_mask = cv2.resize(inst_mask_crop, (1200, 680), interpolation=cv2.INTER_LINEAR)
                else:
                    img = cv2.resize(img_crop, (640, 480), interpolation=cv2.INTER_LINEAR)
                    inst_mask = cv2.resize(inst_mask_crop, (640, 480), interpolation=cv2.INTER_LINEAR)
            path_target = f"{dir_scan_select}/image/{idx:04d}.png"
            mask_target = f"{dir_scan_select}/mask/{idx:04d}.png"
            cv2.imwrite(path_target, img)
            cv2.imwrite(mask_target, inst_mask)
            ...
            # filled image
            if b_with_inpaint:
                path_src = f"{dir_scan}/image_filled/{idx}.jpg"
                path_target = f"{dir_scan_select}/image_filled/{idx:04d}.jpg"
                shutil.copyfile(path_src, path_target)
            
        # GT mesh
        if 'replica' in dir_scan:
            path_gt_mesh = os.path.join(dir_scan, 'mesh.ply')
            assert path_gt_mesh
    ...
    @staticmethod
    def select_data_by_range_for_whole(dir_scan, dir_scan_select, start_id, end_id, interval, b_crop_images, cropped_size = (1248, 936)):  
        '''
        Args:
            b_crop_images: crop images to cropped size if true, and resize cropped images to (640,480)
            cropped_size: (640,480)*1.95
    ...
    def load_and_merge_depth_maps(self, with_obj_mask=False):
        self.depthmaps = self.read_depthmaps(self.dir_depthmap)
        self.num_images = len(glob.glob(f"{self.dir_image}/**.png"))
        
        if with_obj_mask:
            self.dir_maskmap = os.path.join(self.dir_scan, 'mask')
            self.maskmaps = self.read_maskmaps(self.dir_maskmap)
        else:
            self.maskmaps = None
        ...
        if self.depthmaps.shape[0] > 200:
            ...
            if with_obj_mask:
                points = GeometryUtils.fuse_depthmaps(depthmaps_fuse, self.intrinsics_depth, self.poses_w2c[idx_imgs], with_obj_mask=with_obj_mask, masks=self.maskmaps[idx_imgs])
            else:
                points = GeometryUtils.fuse_depthmaps(depthmaps_fuse, self.intrinsics_depth, self.poses_w2c[idx_imgs], with_obj_mask=with_obj_mask)

            self.arr_imgs = (self.read_rgbs(self.dir_image, (640,480))[idx_imgs])
            arr_imgs = self.arr_imgs.reshape(-1,3)
        else:

            points = GeometryUtils.fuse_depthmaps(self.depthmaps, self.intrinsics_depth, self.poses_w2c, with_obj_mask=with_obj_mask, masks=self.maskmaps)
    ...
    def read_maskmaps(self, dir):
        vec_path = sorted(glob.glob(f"{dir}/**.png"))
        mask_maps = []
        for i in range(len(vec_path)):
            img = self.read_one_img(vec_path[i]).astype(np.bool)
            mask_maps.append(img)
    ...
    def generate_neus_data(self, radius_normalize_sphere=1.0, with_obj_mask=False):
        if self.path_cloud_sfm:
            ...
            self.load_and_merge_depth_maps(with_obj_mask=with_obj_mask)
        ...
        trans_n2w, scale = GeometryUtils.get_norm_matrix_from_point_cloud(cloud_clean, radius_normalize_sphere=radius_normalize_sphere)
        ...
        # transform gt mesh
        if 'replica' in self.dir_scan:
            path_gt_mesh = os.path.join(self.dir_scan, 'mesh.ply')
            ...
            GeometryUtils.transform_pointcloud(path_gt_mesh, trans, path_save) 
        else:
