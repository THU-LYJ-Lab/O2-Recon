...
def write_point_cloud(path_save, mesh):
    # assert IOUtils.ensure_dir_existence(os.path.dirname(path_save))
    o3d.io.write_point_cloud(path_save, mesh)
...
def get_world_points(depth, intrinsics, extrinsics, with_obj_mask=False, mask=None):
    '''
    Args:
        depthmap: H*W
...
def fuse_depthmaps(depthmaps, intrinsics, extrinsics, b_normalize = False, with_obj_mask=False, masks=None):
    '''
    args:
    ...
    for i in range(depthmaps.shape[0]):
        ...
        #     depth = scale * depth
        if with_obj_mask:
            points = get_world_points(depth, cam_int, cam_ext, with_obj_mask=with_obj_mask, mask=masks[i])
        else:
            points = get_world_points(depth, cam_int, cam_ext, with_obj_mask=with_obj_mask)
    ...
    # if with_obj_mask:
    #     # create open3d point cloud from points_fuse
    #     o3d_point_cloud = o3d.geometry.PointCloud()
    #     o3d_point_cloud.points = o3d.utility.Vector3dVector(points_fuse)
    #     # remove outlier points
    #     o3d_point_cloud, _ = o3d_point_cloud.remove_statistical_outlier(nb_neighbors=50, std_ratio=2.0)
        
    #     # get points_fuse from o3d point cloud
    #     points_fuse = np.array(o3d_point_cloud.points)
...
# transformation
def transform_pointcloud(path_pointcloud, trans, path_save = None):
    '''Transfrom mesh using the transformation matrix
    Args:
        path_mesh
        trans: 4*4
    Return:
        mesh_trans
    '''
    mesh = read_point_cloud(path_pointcloud)
    mesh_trans = copy.deepcopy(mesh)
    mesh_trans.transform(trans)
    if path_save is not None:
        write_point_cloud(path_save, mesh_trans)
    return mesh, mesh_trans
