...
class Dataset:
    ...
    def __init__(self, conf):
        ...
        depth_lis = sorted(glob(os.path.join(self.data_dir, 'scale_depth/*.png')))
        self.depth_np = np.stack([self.read_depth_img(im_name, self.resolution_level) for im_name in depth_lis])
        ...
        if len(masks_lis) == 0:
            ...
            self.masks_np = np.stack([self.read_img(im_name, self.resolution_level) for im_name in masks_lis])[:,:,:,0] / 255.0
        ...
        if checkExistence(f'{self.data_dir}/bbox.txt'):
            ...
            self.bbox_center = self.bbox_min + (self.bbox_max - self.bbox_min) / 2
        elif len(masks_lis) > 0:
            point_cloud_norm = o3d.io.read_point_cloud(f'{self.data_dir}/point_cloud_scan_norm.ply')

            point_cloud_norm_downsampled = point_cloud_norm.voxel_down_sample(voxel_size=0.01)
            point_cloud_norm_filtered, _ = point_cloud_norm_downsampled.remove_statistical_outlier(nb_neighbors=50, std_ratio=0.1)
            
            o3d.io.write_point_cloud(f'{self.data_dir}/point_cloud_scan_norm_filtered.ply', point_cloud_norm_filtered)
            
            self.bbox_min, self.bbox_max, self.bbox_center = GeoUtils.get_aabb(point_cloud_norm_filtered, scale=1.1)
            # import pdb; pdb.set_trace()
        else:
        ...
        self.b_use_nvt = conf.get('b_use_nvt', False)
        if self.b_use_nvt:  
            self.novel_pose_radius_range, self.novel_pose_angle_std = self.get_novel_pose_params()
            
            self.semantic_label = None
            with open(f'{self.data_dir}/semantic.txt', 'r') as f:
                self.semantic_label = f.readlines()[0].strip()
            
            with torch.no_grad():
                ## calculate CLIP similarity between images and the semantic label.
                self.clip_model, self.clip_preprocess = clip.load("ViT-B/32", device=self.device)
                clip_mean = (0.48145466, 0.4578275, 0.40821073)
                clip_std = (0.26862954, 0.26130258, 0.27577711)
                self.clip_mean = torch.tensor(clip_mean).to(self.device).reshape(1, 3, 1, 1)
                self.clip_std = torch.tensor(clip_std).to(self.device).reshape(1, 3, 1, 1)
                clip_score_file = f'{self.data_dir}/clip_score.json'
                if not os.path.exists(clip_score_file):
                    self.clip_scores = {}
                    for idx,img in enumerate(self.images_np):
                        # import pdb; pdb.set_trace()
                        
                        img_mask = self.masks_np[idx]
                        ...
                        img_square_bbox = self.get_square_bbox(img_bbox, (img.shape[1], img.shape[0]))
                        
                        img = img[img_square_bbox[1]:img_square_bbox[3], img_square_bbox[0]:img_square_bbox[2], :]
                        
                        
                        pil_image = Image.fromarray(cv.cvtColor((img*255).astype(np.uint8), cv.COLOR_BGR2RGB))
                        processed_img = self.clip_preprocess(pil_image).unsqueeze(0).to(self.device)
                        img_feature = self.clip_model.encode_image(processed_img)
                        text_feature = self.clip_model.encode_text(clip.tokenize(self.semantic_label).to(self.device))
                        
                        self.clip_scores[idx] = torch.cosine_similarity(img_feature, text_feature).item()
                        
                    with open(clip_score_file, 'w') as f:
                        json.dump(self.clip_scores, f)
                else:
                    with open(clip_score_file, 'r') as f:
                        self.clip_scores = json.load(f)
                    
                    assert len(self.clip_scores) == len(self.images_np)

                    ## replace the keys from str to int
                    self.clip_scores = {int(k):v for k,v in self.clip_scores.items()}
                    
                # import pdb; pdb.set_trace()
                
                # ## save the images in the order of clip score
                # clip_order_image_dir = f'{self.data_dir}/clip_order_images'
                # if not os.path.exists(clip_order_image_dir):
                #     os.makedirs(clip_order_image_dir)
                # sorted_clip_scores = sorted(self.clip_scores.items(), key=lambda x: x[1], reverse=True)
                # for idx, (img_idx, score) in enumerate(sorted_clip_scores):
                #     cv.imwrite(f'{clip_order_image_dir}/{idx:03d}_{score:.3f}_{img_idx:03d}.png', (self.images_np[img_idx]*255).astype(np.uint8))
                # import pdb; pdb.set_trace()                
                
                
                # ## cluster the camera poses according to ratation quaternion
                # ## import Quaternion module
                # from pyquaternion import Quaternion
                        
                # quatanions_all = []
                # for pose in self.pose_all:
                #     rotation_mat = pose[:3,:3].cpu().numpy()
                    
                #     rotation_quat = Quaternion(matrix=rotation_mat, atol=1e-07)
                ...
                # clustering_selection_path = f'{self.data_dir}/clustering_selection'
                # if not os.path.exists(clustering_selection_path):
                #     os.makedirs(clustering_selection_path)
                
                # for idx_cluster in range(n_clusters):
                #     clustered_img_idx = np.array(range(self.n_images))[clustered_quatanions.labels_ == idx_cluster]
                #     clustered_img_idx_and_scores[idx_cluster] = {}
                #     for ii in clustered_img_idx:
                #         clustered_img_idx_and_scores[idx_cluster][ii] = self.clip_scores[ii]                 
                ...
                #     ## find top-2 images from each cluster
                #     # for idx, (img_idx, score) in enumerate(clustered_img_idx_and_scores[idx_cluster][:2]):
                #     img_idx = clustered_img_idx_and_scores[idx_cluster][0][0]
                #     score = clustered_img_idx_and_scores[idx_cluster][0][1]
                #     cv.imwrite(f'{clustering_selection_path}/{idx_cluster:03d}_{score:.3f}_{img_idx:03d}.png', (self.images_np[img_idx]*255).astype(np.uint8))
            
                # import pdb; pdb.set_trace()
                
                ## find the index of highest clip score
                self.reference_img_idx = max(self.clip_scores, key=self.clip_scores.get)
                
                self.reference_img_np = self.images_np[self.reference_img_idx]
                print("max clip score: ", self.clip_scores[self.reference_img_idx])
                print("reference image index: ", self.reference_img_idx)

                self.nvt_resolution_level = conf.get('nvt_resolution_level', 3)
                
                ## get the reference image at resolution level
                reference_img_np_uint8, reference_normal_np_uint8 = self.get_reference_image(resolution_level=self.nvt_resolution_level)
                
                # import pdb; pdb.set_trace()
                
                # pil_reference_img = Image.fromarray(cv.cvtColor(reference_img_np_uint8, cv.COLOR_BGR2RGB), mode='RGB')
                # processed_reference_img = self.clip_preprocess(pil_reference_img).unsqueeze(0).to(self.device)
                # print(processed_reference_img[0,:,104,104])
                
                
                processed_reference_img = self.clip_preprocess_torch_api(torch.tensor(reference_img_np_uint8))   
                self.reference_color_img_feature = self.clip_model.encode_image(processed_reference_img)
                
                processed_reference_normal = self.clip_preprocess_torch_api(torch.tensor(reference_normal_np_uint8))
                self.reference_normal_map_feature = self.clip_model.encode_image(processed_reference_normal)
                
                self.text_feature = self.clip_model.encode_text(clip.tokenize(self.semantic_label).to(self.device))
    
    def clip_preprocess_torch_api(self, img):
        '''
        receives unint8 images
        '''
        img = img.to(self.device).unsqueeze(0).float().permute(0, 3, 1, 2)
        img = F.interpolate(img, size=(224,224), mode='bicubic')
        img = torch.flip(img, dims=[1])
        img = ((img / 255) - self.clip_mean) / self.clip_std
        return img
    
    def get_square_bbox(self, bbox, image_shape, scale=1.0):
        '''
    ...
    def get_reference_image(self, resolution_level=4):
        H, W = self.reference_img_np.shape[0], self.reference_img_np.shape[1]
        reference_img_np_resize = cv.resize((self.reference_img_np*255).astype(np.uint8), (W//2**resolution_level, H//2**resolution_level))
        
        reference_normal_np = (((self.normals_np[self.reference_img_idx]+1)*0.5).clip(0,1) * 255).astype(np.uint8)
        reference_normal_np_resize = cv.resize(reference_normal_np, (W//2**resolution_level, H//2**resolution_level))
        
        ## mask the reference image
        img_mask = self.masks_np[self.reference_img_idx]
        ## [min_x, min_y, max_x, max_y]
        img_bbox = np.array([np.min(np.where(img_mask)[1]), np.min(np.where(img_mask)[0]), np.max(np.where(img_mask)[1]), np.max(np.where(img_mask)[0])])
        img_bbox = img_bbox // (2**resolution_level)
        ## modify the bbox to square shape
        img_square_bbox = self.get_square_bbox(img_bbox, (reference_img_np_resize.shape[1], reference_img_np_resize.shape[0]))
        
        reference_img_np_resize = reference_img_np_resize[img_square_bbox[1]:img_square_bbox[3], img_square_bbox[0]:img_square_bbox[2], :]
        
        reference_normal_np_resize = reference_normal_np_resize[img_square_bbox[1]:img_square_bbox[3], img_square_bbox[0]:img_square_bbox[2], :]
        
        return reference_img_np_resize, reference_normal_np_resize
    ...
    def initialize_patchmatch(self):
        ...
        self.extrinsics_all = torch.from_numpy(np.linalg.inv(self.pose_all.cpu().numpy())).cuda()
        self.images_gray = []
    ...
    def read_depth_img(self, path, resolution_level):
        img = cv.imread(path, -1).astype(np.float32)
        img /= 1000.
        H, W = img.shape[0], img.shape[1]
    ...
    def get_novel_pose_params(self):
        # get object center
        obj_center = torch.from_numpy(self.bbox_center).to(self.device)
        
        # get radius range according to bbox_max and bbox_min
        diag_from_bbox = (self.bbox_max - self.bbox_min)
        radius_from_bbox = np.linalg.norm(diag_from_bbox) / 2.0
        
        # get radius range from all camera poses
        camera_distance = []
        for pose in self.pose_all:
            cam_position = pose[:3, 3]
            distance_from_cam = torch.norm(cam_position - obj_center)
            camera_distance.append(distance_from_cam)
        radius_from_camera_max = max(camera_distance)
        radius_from_camera_min = min(camera_distance)
        
        radius_range = (min(radius_from_bbox, radius_from_camera_min), max(radius_from_bbox, radius_from_camera_max)*0.9)
        
        pose_sample_std = 0.5
        
        return radius_range, pose_sample_std
    
    def get_novel_pose(self, img_idx):
        
        this_pose_position = self.pose_all[img_idx][:3, 3]
        object_center = torch.from_numpy(self.bbox_center).to(self.device)
        
        view_vector = object_center - this_pose_position
        
        # calculate yaw and pitch angle according to view vector
        # x: forward, y: left, z: up
        yaw = torch.atan2(view_vector[1], view_vector[0])
        pitch = torch.atan2(torch.norm(view_vector[[0, 1]]), view_vector[2])
        
        # sample new yaw and pitch angles for novel pose from gaussian distribution with mean = yaw and pitch
        new_yaw = torch.normal(yaw, self.novel_pose_angle_std)
        new_pitch = torch.normal(pitch, self.novel_pose_angle_std)
        
        # calculate new view vector [x,y,z]
        new_view_vector = torch.tensor([torch.cos(new_yaw) * torch.sin(new_pitch), torch.sin(new_yaw) * torch.sin(new_pitch), torch.cos(new_pitch)])
        new_cam_position = object_center - new_view_vector * (torch.rand(1)*(self.novel_pose_radius_range[1]-self.novel_pose_radius_range[0]) + self.novel_pose_radius_range[0])
        
        # calculate new pose
        new_view_vector = new_view_vector[None, :]
        new_cam_position = new_cam_position[None, :]
        new_pose = self.create_cam2world_matrix(new_view_vector, new_cam_position)
        
        new_pose = new_pose[0]
        
        return new_pose
    
    def create_cam2world_matrix(self, forward_vector, origin):
        """
        Takes in the direction the camera is pointing and the camera origin and returns a cam2world matrix.
        Works on batches of forward_vectors, origins. Assumes z-axis is up and that there is no camera roll.
        
        The cam2world rotation consists of : [right_vec, down_vec, forward_vec], such that cam2world_rot*[x,y,z]^T corresponds to the world coordinates of [x,y,z]^T.
        """        
        
        forward_vector = F.normalize(forward_vector, dim=-1)
        up_vector = torch.tensor([0, 0, 1], dtype=forward_vector.dtype, device=origin.device).expand_as(forward_vector)

        right_vector = torch.cross(forward_vector, up_vector, dim=-1)
        right_vector = F.normalize(right_vector, dim=-1)
        
        down_vector = torch.cross(forward_vector, right_vector, dim=-1) 
        down_vector = F.normalize(down_vector, dim=-1)
    ...
    def gen_rays_at_novel_pose(self, img_idx, pose = None, resolution_level=1):
        pose_cur = self.get_novel_pose(img_idx)
        # pose_cur = self.pose_all[img_idx]
        
        ## get 2D bbox of novel view, by projecting the corners of 3D bbox to this pose.
        ## 1. get the 8 corners according to the bbox_min and bbox_max
        corners_2D_list = []
        corners_3D_list = []
        image_intrinsic = self.intrinsics_all[img_idx]
        image_extrinsic = torch.inverse(pose_cur)
        ...
        for ii in range(2):
            for jj in range(2):
                for kk in range(2):
                    this_corner_3D = self.bbox_min + np.array([ii, jj, kk]) * (self.bbox_max - self.bbox_min)
                    
                    this_corner_3D = torch.tensor(this_corner_3D, dtype=torch.float32, device=pose_cur.device)[:,None]
                    
                    this_corner_3D = torch.matmul(image_extrinsic[:3, :3], this_corner_3D) + image_extrinsic[:3, 3:]
                    
                    corners_3D_list.append(this_corner_3D)
                    
                    this_corner_2D = torch.matmul(image_intrinsic[:3,:3], this_corner_3D)
                    
                    this_corner_2D = this_corner_2D[:2] / this_corner_2D[2]
                    
                    corners_2D_list.append(this_corner_2D)
        
        corners_3D_list = torch.stack(corners_3D_list)
        
        bbox_2D_min = torch.stack(corners_2D_list)[:,:].min(0)[0].int()
        bbox_2D_max = torch.stack(corners_2D_list)[:,:].max(0)[0].int()            

        l = resolution_level
        bbox_W = min(bbox_2D_max[0].item(), self.W - 1) - max(0, bbox_2D_min[0].item())
        bbox_H = min(bbox_2D_max[1].item(), self.H - 1) - max(0, bbox_2D_min[1].item())
        tx = torch.linspace(max(0, bbox_2D_min[0].item()), min(bbox_2D_max[0].item(), self.W - 1), bbox_W // l)
        ty = torch.linspace(max(0, bbox_2D_min[1].item()), min(bbox_2D_max[1].item(), self.H - 1), bbox_H // l)
        ...
        return rays_o.transpose(0, 1), rays_v.transpose(0, 1), corners_3D_list
    ...
    def random_get_rays_at(self, img_idx, batch_size, pose = None):
        ...
        point_a = torch.stack([torch.arange(0, self.W) for _ in range(self.H)])
        point_b = torch.stack([torch.arange(0, self.H) for _ in range(self.W)]).T
        point = torch.stack([point_b, point_a], dim=-1)
        point = point[self.normal_mask[img_idx]]
        rand_idx = torch.randint(low=0, high=point.shape[0], size=[batch_size - batch_size // 2])
        point = point[rand_idx]

        pixels_x = point[:, 1]
        pixels_y = point[:, 0]
        
        rand_pixels_x = torch.randint(low=0, high=self.W, size=[batch_size // 2])
        rand_pixels_y = torch.randint(low=0, high=self.H, size=[batch_size // 2])

        pixels_x = torch.cat([pixels_x, rand_pixels_x])
        pixels_y = torch.cat([pixels_y, rand_pixels_y])
        ...
        # if self.b_use_nvt:
        #     img_mask = self.masks_np[img_idx]
        #     ## [min_x, min_y, max_x, max_y]
        #     img_bbox = np.array([np.min(np.where(img_mask)[1]), np.min(np.where(img_mask)[0]), np.max(np.where(img_mask)[1]), np.max(np.where(img_mask)[0])])
        #     bbox_mask = torch.zeros_like(self.masks[img_idx])
        #     bbox_mask[img_bbox[1]:img_bbox[3], img_bbox[0]:img_bbox[2]] = 1
        #     mask = bbox_mask[(pixels_y, pixels_x)][:,None]
            
        
        depth_sample = self.depth[img_idx][(pixels_y, pixels_x)][:,None].cuda()
        p = torch.stack([pixels_x, pixels_y, torch.ones_like(pixels_y)], dim=-1).float()  # batch_size, 3
        p = torch.matmul(self.intrinsics_all_inv[img_idx, None, :3, :3], p[:, :, None]).squeeze() # batch_size, 3
        rays_v = p / torch.linalg.norm(p, ord=2, dim=-1, keepdim=True)    # batch_size, 3
        ...
        if self.use_normal:
            ...
            normal_mask = self.normal_mask[img_idx][(pixels_y, pixels_x)].cuda()
        ...
        return torch.cat([rays_o.cpu(), rays_v.cpu(), color, mask], dim=-1).cuda(), pixels_x, pixels_y, normal_sample, planes_sample, subplanes_sample, normal_mask, depth_sample    # batch_size, 10
