...
def compute_homography(pt_ref, n_ref, K, extrin_ref, extrin_src):
    ...
    ref_pose = torch.from_numpy(np.linalg.inv(extrin_ref.cpu().numpy())).cuda()
    ...
    mat_homography = (K[None, ...] @ (R_rel[None, ...] + t_rel[None,...] @ n_ref.reshape(num_pts,1,3) / d))@ torch.from_numpy(np.linalg.inv(K.cpu().numpy())).cuda()
