...
class NeuSLoss(nn.Module):
    def __init__(self, conf):
        ...
        self.sdf_loss_weight = conf.get('sdf_loss_weight', 0.0)
        self.freespace_loss_weight = conf.get('freespace_loss_weight', 0.0)
    ...
    def forward(self, input_model, render_out, sdf_network_fine, patchmatch_out = None):
        ...
        if patchmatch_out is not None:
            if patchmatch_out['patchmatch_mode'] == 'use_geocheck':
                ...
                normal_mask = input_model['normal_mask'].bool()
            else:
        ...
        if True:
            ...
            color_fine_loss = F.l1_loss(color_error, torch.zeros_like(color_error), reduction='sum') / (mask_sum + 1e-5)
            ...
            # check nan in weight_sum
            # if torch.isnan(weight_sum).any():
            #     import pdb; pdb.set_trace()
            
            background_loss = F.binary_cross_entropy(weight_sum.clip(1e-3, 1.0 - 1e-3), mask.clip(0,1))
        ...
        # SDF and Free-space loss
        sdf_loss, freespace_loss = 0.0, 0.0
        if self.sdf_loss_weight > 0 and self.freespace_loss_weight > 0:
            depth_gt = input_model['depth_sample']
            # import pdb; pdb.set_trace()
            freespace_loss, sdf_loss = get_sdf_loss(mid_z_vals, depth_gt, predicted_sdf.view(mid_z_vals.shape), 0.16)
        ...
        if self.normal_weight > 0 and normals_target is not None:
            ...
            normal_mask = input_model['depth_sample'] > 0
            
            normal_certain_weight = normal_certain_weight*normal_mask
        ...
        if self.depth_weight > 0:
            ...
            depth_error = (depth - depth_gt) * depth_mask
            depths_fine_loss = F.l1_loss(depth_error, torch.zeros_like(depth_error), reduction='sum') / (mask_sum + 1e-5)
        ...
        loss = color_fine_loss * self.color_weight +\
                ...
                freespace_loss * self.freespace_loss_weight
