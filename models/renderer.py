...
class NeuSRenderer:
    ...
    def get_color(self, v):
        ...
        while idx < v.shape[0]:
            ...
            viewing_dir = -vertices / torch.linalg.norm(vertices, ord=2, dim=-1, keepdim=True)
            viewing_dir = -gradients
            sampled_color = self.color_network_fine(vertices, gradients, viewing_dir, feature_vector).reshape(-1, 3)
            sampled_color_list.append(sampled_color.detach().cpu().numpy())
            idx += l
        
        if len(sampled_color_list) == 0:
            return None
        else:
